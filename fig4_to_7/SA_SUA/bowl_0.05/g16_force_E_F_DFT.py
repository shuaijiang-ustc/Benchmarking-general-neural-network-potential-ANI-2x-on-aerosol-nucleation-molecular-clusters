	...
	for i, line in enumerate(lines):
		...
		if line.count('Forces (Hartrees/Bohr)') > 0:
			...
			for j in range(1, len(crds)+1):
				...
				k += 1
			break
	f1=open("Energy_Hatree.txt","a+")
	f1.write(str(Energy))
	f1.write('\n')
	f1.close()	
	Forces_unit=-Forces*JOULEPERHARTREE*BOHRPERA# Hatree/Bohr => J/mol
	with open('Force_J_per_mol_Nuclear_charge.txt', 'a') as f:
		f.write(str(len(atoms))+'\n')
		for item in atoms:
			f.write(str(item)+' ')
		f.write('\n')
	f.close()
	with open('Force_J_per_mol_Nuclear_charge.txt', 'ab') as f:
...
for i in range(len(xyz_atoms_objs)):  
	G16_PM7(xyz_atoms_objs[i].get_positions(), xyz_atoms_objs[i].get_atomic_numbers(), basis_ = '6-31++G(d,p)', xc_='wB97XD', jobtype_='force', threads=32)