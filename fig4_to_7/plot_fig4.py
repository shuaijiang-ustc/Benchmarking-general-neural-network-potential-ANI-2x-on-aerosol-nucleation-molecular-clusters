...
for dirt in dir_list:
	...
	for line in f.readlines():
		...
		RE_PM7.append(float(line.split()[2]))
	f.close()
	RMSE_RE = metrics.mean_squared_error(RE_ANI, RE_DFT)**0.5
	RMSE_RE_PM7 = metrics.mean_squared_error(RE_PM7, RE_DFT)**0.5
	if i==1:
	...
	ax1.scatter(RE_DFT, RE_PM7, marker="o", color=color_other[1], s=markersize_value)
	ax1.xaxis.set_major_locator(x_major_locator)
	ax1.yaxis.set_major_locator(y_major_locator)
	axis_low=min(np.min(RE_DFT), np.min(RE_ANI), np.min(RE_PM7))
	axis_high=max(np.max(RE_DFT), np.max(RE_ANI), np.max(RE_PM7))
	...
	ax1.text(0.015,0.85, 'RMSE_PM7=%s kcal/mol'%format(RMSE_RE_PM7,'.2f'), transform=ax1.transAxes,fontsize=fontsize)
	plt.xlabel('RE from DFT (kcal/mol)')
	plt.ylabel('RE from ANI-2x/PM7 (kcal/mol)')

fig1.legend(ax_set, labels=["ANI-2x", "PM7"],loc="upper right")

plt.tight_layout()	
plt.savefig('./fig4_SP_on_MetaMD.jpeg',dpi=dpi_numb)	